from PyQt6.QtCore import QRectF

from src.Scene.Game.Digit import Digit
from src.Scene.Game.Display import Display
from src.TextInForeground import TextInForeground


class Scorer(Display):

    def __init__(self, name_left, name_right):
        super(Scorer, self).__init__()

        # instantiate children items

        self.text_left = self.text_displayed(name_left, self.max_width / 2 - 2 * self.marge_width)
        self.text_right = self.text_displayed(name_right, self.max_width / 2 - 2 * self.marge_width)
        self.ten_digit_left = Digit()
        self.unit_digit_left = Digit()
        self.ten_digit_right = Digit()
        self.unit_digit_right = Digit()

        # set geometry

        width = max(
            self.ten_digit_left.boundingRect().width() * 4 + self.marge_width * 7,
            self.text_left.boundingRect().width() + self.text_right.boundingRect().width() + self.marge_width * 3
        )
        print("scorer:",
              "w: ", width,
              "digitw: ", self.ten_digit_left.boundingRect().width(),
              "maxw: ", self.max_width)
        height = self.ten_digit_left.boundingRect().height() \
                 + self.text_left.boundingRect().height() \
                 + 3 * self.marge_height

        self.rect = QRectF(0, 0, width, height)

        # position items

        self.text_left.setPos(
            (width / 2 - 2 * self.marge_width - self.ten_digit_left.boundingRect().width()) / 2,
            self.marge_height)

        self.text_right.setPos(
            (width / 2 + 2 * self.marge_width + self.ten_digit_left.boundingRect().width()) / 2,
            self.marge_height)

        y = self.text_left.boundingRect().height() + 2 * self.marge_height

        self.ten_digit_left.setPos(self.marge_width, y)
        self.unit_digit_left.setPos(self.ten_digit_left.boundingRect().width() + 2 * self.marge_width, y)

        self.ten_digit_right.setPos(width - 2 * (self.marge_width + self.ten_digit_left.boundingRect().width()), y)
        self.unit_digit_right.setPos(width - self.marge_width - self.ten_digit_left.boundingRect().width(), y)

    def set_score_left(self, number):
        self.ten_digit_left.display_number(number // 10)
        self.unit_digit_left.display_number(number % 10)

    def set_score_right(self, number):
        self.ten_digit_right.display_number(number // 10)
        self.unit_digit_right.display_number(number % 10)


