...
class Statistics:
    ...
    def add_card_to_autoside(self, side):
        if side.nCard == 1:
            self.auto_ls1.append(side.numero)
            self.auto_ls1.sort()
            for i in range(len(self.auto_ls0)):
                if self.auto_ls0[i] == side.numero:
                    del self.auto_ls0[i]
                    break

        elif side.nCard == 2:
            for i in range(len(self.auto_ls1)):
                if self.auto_ls1[i] == side.numero:
                    del self.auto_ls1[i]
                    break

            self.auto_ls2.append(side.numero)
            self.auto_ls2.sort()

        else:
            for i in range(len(self.auto_ls)):
                if self.auto_ls[i] == side.numero:
                    del self.auto_ls[i]
                    break

            for i in range(len(self.auto_ls2)):
                if self.auto_ls2[i] == side.numero:
                    del self.auto_ls2[i]
                    break
